...
# 新增一级科室
mysql = PlSqlDb()
sql_1 = '''select max(id),max(dept_id) from system.standard_department;'''
res_1 = mysql.query(sql_1)
max_id = res_1[0][0]
max_dept_id = res_1[0][1]
max_sort_no = res_1[0][2]
dept_name = ''
...
for i in range(1,37):
    ...
    max_sort_no = max_sort_no + 1
    dept_name = '最大层级' + str(i)
    ...
    values ({0}, {1}, '{2}', {3}, '{4}', 3, '市级重点专科', 2, 'erke', null, 1, '2021-01-28 03:28:57.698057', {5}, null);'''.format(
        max_id, max_dept_id, dept_name,dept_type_id,dept_type_name,max_sort_no)
